...
public class GirlAnimation : MonoBehaviour {
	...
	void Update () {
		if (Hero.GetComponent<PlayerController> ().Locked) {
			GetComponent<Renderer>().material.mainTextureOffset = new Vector2(1 * FrameOffsetX, FrameOffsetsY[1]);
		}
		else
		{
			if (!Hero.GetComponent<PlayerController>().Dead)
			{
				up = Input.GetKeyDown(KeyCode.UpArrow);
				down = Input.GetKeyDown(KeyCode.DownArrow);
				right = Input.GetKey(KeyCode.RightArrow)
					|| GameSettings.Hero.GetComponent<PlayerController>().AlwaysRun;
				...
				*/
				...
				if (Hero.GetComponent<PlayerController>().jump)
					GetComponent<Renderer>().material.mainTextureOffset = new Vector2(1 * FrameOffsetX, FrameOffsetsY[10]);

				if (! (right || left || Hero.GetComponent<PlayerController>().jump))
					GetComponent<Renderer>().material.mainTextureOffset = new Vector2(1 * FrameOffsetX, FrameOffsetsY[1]);
			}
			else
			{
